...
export const profileRouter = router({
  getByWallet: publicProcedure
    ...
    .query(async ({ input, ctx }) => {
      return getOrSet(
        CacheKeys.profile(input.wallet),
        TTL.profile,
        async () => {
  ...
  getByUsername: publicProcedure
    ...
    .query(async ({ input, ctx }) => {
      return getOrSet(
        CacheKeys.profileByUsername(input.username),
        TTL.profile,
        async () => {
          const [profile] = await ctx.db
            .select()
  ...
  getFollowing: publicProcedure
    ...
    .query(async ({ input, ctx }) => {
      const rows = await ctx.db
        ...
        .where(
          sql`${eq(follows.followerWallet, input.wallet)}${input.cursor ? sql` AND ${follows.createdAt} < ${new Date(input.cursor)}` : sql``}`
        )
        .orderBy(desc(follows.createdAt))
        .limit(input.limit + 1);
