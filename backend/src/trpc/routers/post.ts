    ...
    .query(async ({ input, ctx }) => {
      ...
      return getOrSet(cacheKey, TTL.feed, async () => {
        ...
        const rows = await ctx.db
          ...
          .from(posts)
          .where(
            sql`${posts.authorWallet} IN (${followedWallets})${input.cursor ? sql` AND ${posts.createdAt} < ${new Date(input.cursor)}` : sql``}`
          )
  ...
  getByAuthor: publicProcedure
    ...
    .query(async ({ input, ctx }) => {
      const rows = await ctx.db
        ...
        .from(posts)
        .where(
          sql`${eq(posts.authorWallet, input.wallet)}${input.cursor ? sql` AND ${posts.createdAt} < ${new Date(input.cursor)}` : sql``}`
        )
        .orderBy(desc(posts.createdAt))
        .limit(input.limit + 1);
  ...
  getComments: publicProcedure
    ...
    .query(async ({ input, ctx }) => {
      const rows = await ctx.db
        ...
        .from(comments)
        .where(
          sql`${eq(comments.postAddress, input.postAddress)}${input.cursor ? sql` AND ${comments.createdAt} < ${new Date(input.cursor)}` : sql``}`
        )
