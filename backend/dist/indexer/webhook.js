...
import { profiles, posts, likes, follows, tips, comments, notifications, communities, } from "../db/schema.js";
...
function verifyHeliusSignature(body, signature, secret) {
    if (!signature)
        return false;
...
function classifyEvent(tx) {
    ...
    if (log.includes("CreateProfile"))
        return "PROFILE_CREATED";
    if (log.includes("CreatePost"))
        return "POST_CREATED";
    if (log.includes("CreateComment"))
        return "COMMENT_CREATED";
    if (log.includes("LikePost"))
        return "LIKE";
    if (log.includes("FollowUser"))
        return "FOLLOW";
    if (log.includes("SendTip"))
        return "TIP";
    if (log.includes("CreateCommunity"))
        return "COMMUNITY_CREATED";
    return "UNKNOWN";
}
// ─── Event Handlers ───────────────────────────────────────────────────────────
async function handleProfileCreated(tx) {
...
async function handleCommentCreated(tx) {
    ...
    if (!authorWallet || !commentAddress || !postAddress)
        return;
    await db
        .insert(comments)
        .values({
...
async function handleFollow(tx) {
    ...
    await invalidate(CacheKeys.profile(followerWallet), CacheKeys.profile(followeeWallet));
    await publish(`profile:${followeeWallet}`, { type: "FOLLOW", actorWallet: followerWallet });
}
async function handleTip(tx) {
...
// ─── Main Handler ─────────────────────────────────────────────────────────────
export async function heliusWebhookHandler(c) {
